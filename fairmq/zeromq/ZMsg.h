...
struct ZMsg
{
    ...
    explicit ZMsg(std::size_t size) noexcept
    {
        [[maybe_unused]] auto const rc = zmq_msg_init_size(Msg(), size);
        assertm(rc == 0, "msg init successful");   // NOLINT
    }
    ...
    {
        [[maybe_unused]] auto const rc = zmq_msg_init_data(Msg(), data, size, freefn, hint);
        assertm(rc == 0, "msg init successful");   // NOLINT
    }
    ~ZMsg() noexcept
    {
    ...
    int Send(void* socket, int flags) noexcept
    {
        assertm(!fSent, "msg not yet sent");
        auto const nbytes = zmq_msg_send(Msg(), socket, flags);
        fSent = nbytes > 0;
        return nbytes;
    }
